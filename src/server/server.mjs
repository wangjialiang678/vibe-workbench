...
function handleRequest(req, res, expectedToken = '', eventWebhook = '', participantsFile = DEFAULT_PARTICIPANTS_FILE, runtimeState = { workerHeartbeat: null }) {
  const method = req.method.toUpperCase(); const rawUrl = req.url || '/'; const requestUrl = new URL(rawUrl, 'http://localhost'); const urlPath = requestUrl.pathname;
  ...
  catch (error) { console.error('[workbench:participants] 名册读取失败：', error.message); if (isApi) json(res, 500, { ok: false, error: '参与者名册无法读取，请联系管理员' }); else { cors(res); res.writeHead(500, { 'Content-Type': 'text/plain; charset=utf-8' }); res.end('参与者名册无法读取，请联系管理员'); } return; }
  ...
  const identity = auth?.identity || OWNER_IDENTITY; const requestToken = auth?.token || ''; req.identity = identity;
  ...
  const ctx = { req, res, method, rawUrl, requestUrl, urlPath, identity, requestToken, expectedToken, eventWebhook, participantsFile, runtimeState, json, readBody, readRawBody, readRawBodyLimited, parseQuery, cors, noReferrer };
