...
import { displayState, badgeFor, externalBadgeFor, errorRetryable } from '../protocol/status.mjs';
...
export function statusBadgeHtml(statusResp, now) {
  ...
  const externalAudience = hasNestedStatus && statusResp.audience === 'external';
  ...
  const internalBadge = workerOnline
    ? {
        icon: '',
  ...
  const badge = externalAudience ? externalBadgeFor(displ) : internalBadge;

  const driverSourceNote = !externalAudience && status?.driverSource === 'sdk-fallback'
    ? `<span class="driver-source-note">${escHtml(SDK_FALLBACK_NOTICE)}</span>`
    : '';

  // 错误人话提示（P1：按 error.kind 显示 userMessage/suggestedAction）
  let errorDetail = '';
  if (!externalAudience && displ === 'error' && error) {
    const err = error;
  ...
  let retryBtn = '';
  if (!externalAudience && badge.retry === 'force') {
    // 处理中：仅「强制重试」（二次确认）
