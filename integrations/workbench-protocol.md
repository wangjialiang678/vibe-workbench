...
1. 需要对多个相对独立、较零散的事项逐条选择、表态或批注，聊天会让反馈难以对应；
2. 判断本身明显受益于可视化呈现，例如需要对照多组方案、图上落点、界面原型或跨轮差异；
3. 用户明确要求使用工作台。

即使使用工作台，它也只是结构化反馈界面，不能替代完整交付：完整结论仍应在对话或项目文档中给出。拿不准时，留在对话里。

**不用**：完整且连续的交付、简单判断、单个问题、纯闲聊、普通问答，以及无需对方逐项输入的输出。
