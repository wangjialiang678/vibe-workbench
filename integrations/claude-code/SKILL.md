...
- **体验类决策（布局/交互/视觉）必须配可视化原型**（2026-07-23 创始人拍板为跨项目默认）：每个候选方案给 `prototype` 低保真线框或图片，推荐方案再给高保真（HTML mockup 截图 / iframe，手机场景 `frame:"phone"`）；图上开放批注、表态走相邻 verdict/choice。**严禁 ASCII/框线字符画**（"你已经是在 html 里了"，比例字体下必散架，lint `ascii-art` 会警告）；流程示意用 mermaid diagram 块。
