---
name: workbench
description: 用网页工作台（Vibe Workbench）处理多个相对独立事项的逐条反馈，或明显受益于可视化对照、图上落点和跨轮差异的判断；用户明确要求“用工作台”、Vibe Workbench、present 或 /workbench 时也使用。不要用于完整连续交付、简单判断、单个问题或普通评审／共创；工作台只承载结构化反馈，不替代对话或项目文档中的完整结论。
---
...
**默认留在对话里。** 完整分析、完整方案、总结、文稿等连续交付物，直接在对话或项目文档中交付；简单判断、单个选择和一两句即可说明的问题，也直接在对话里处理。不能仅因为出现“选择”“确认方案”“评审”“共创”等词，就自动切换到工作台。

**用**：多个相对独立、较零散的事项需要逐条选择／表态／批注；判断明显受益于方案对照、图上落点、界面原型或跨轮差异；或者用户明确要求使用工作台。

即使使用，工作台也只是结构化反馈界面，不能替代完整交付。拿不准时，留在对话里。

**不用**：完整且连续的交付、简单判断、单个问题、普通问答，以及无需用户逐项输入的输出。
...
- **不可逆/高后果决定用 `choice.mode:"blind"`**：提交前不显示推荐，采集 prediction/premises；两型的 background/why/pros/cons 完整度必须相同（D36）。
...
- **回答者是外部人（顾问/客户）且条目涉及其自己的产物时，每条必须引用对方产物的原文+位置**（2026-09-07 创始人反馈，TMS 柏甫 16 条标签清单病例：只写字段名，"她也可能记不住"自己写过什么）：作者本人也未必记得自己文档里的每个字段——条目要贴出"您的文档某篇:某行原文「……」+ 我们核查的结果"，标准从"没跟过项目的人能看懂"升级为**"回答者不翻任何文件就能作答"**。同场景补两块常配内容：①把判定所依据的**总规则**（如需求源优先级）单独成块讲清并请对方表态知悉——对方不知道规则就无法理解每条判定；②**我方认为对方设计不合理的点单独立块**明说（与出处问题分开问），不要只问事实不亮观点。
