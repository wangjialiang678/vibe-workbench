...
4. **归一化白名单**：归一化 body 里的 `ts`、`assetsVersion`，**以及天然易变的响应头 `Date`**（两服务器跨秒即不同，不归一化会偶发假阳性——2026-08-30 phase1 验收发现）；其余 header、status、body、**连接终止方式**必须一致。响应体内 url 用请求 Host 头拼装（设固定 `ab.fixture.test`）故端口不漏，无需额外归一。
