...
同一轮有至少三个选项完全相同的 `choice`，或一个 `checklist` 至少有三条时，渲染层显示次级批量条。批量动作只补尚未作答的块/条目，不覆盖已有逐条选择；生成的 draft 与 feedback item 和逐条点选使用同一结构。历史轮不渲染批量条，状态晚到而进入只读时也会立即隐藏。

提交人候选来自公开参与者名册。名册为空时，展示/指派层注入 `{ id: "owner", name: "Michael", role: "owner" }` 并默认选中；它不写入 `participants.json`、没有 token，也不进入 magic-link 鉴权。非空名册保持原行为。
